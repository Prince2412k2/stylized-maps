...
export async function buildApp(dependencies) {
    ...
    app.get("/api/v1/search", {
        ...
        if (!dependencies.apiKey)
            return serviceUnavailable(reply, request.id);
        ...
        if (query.proximity && !proximity)
            return invalid(reply, request.id, "proximity must be lng,lat within India");
        if (proximity) {
        ...
        return {
            results: results.flatMap((entry) => {
                if (!isObject(entry) || !isObject(entry.properties) || !isObject(entry.geometry) || !Array.isArray(entry.geometry.coordinates))
                    return [];
                const properties = entry.properties;
                const location = coordinates([entry.geometry.coordinates])[0];
                if (!location || typeof properties.label !== "string")
                    return [];
                const label = typeof properties.name === "string" ? properties.name : properties.label.split(",")[0];
                const context = [properties.locality, properties.region, properties.country].filter((part) => typeof part === "string" && part !== label).join(", ");
                return [{
    ...
    app.get("/api/v1/routes/driving", {
        ...
        if (!start || !end)
            return invalid(reply, request.id, "start and end must be lng,lat coordinates within India");
        ...
        if (!properties || !geometry)
            return notFound(reply, request.id, "No driving route was found");
...
async function providerJson(fetcher, url, timeout, reply, requestId, init = {}) {
    try {
...
function parseCoordinate(value) {
    ...
    if (longitude < INDIA_BOUNDS.west || longitude > INDIA_BOUNDS.east || latitude < INDIA_BOUNDS.south || latitude > INDIA_BOUNDS.north)
        return;
...
function routeCoordinates(geometry) {
    ...
    if (geometry.type === "MultiLineString" && Array.isArray(geometry.coordinates))
        return geometry.coordinates.flatMap((line) => Array.isArray(line) ? coordinates(line) : []);
    return [];
}
function coordinates(values) {
    return values.flatMap((coordinate) => Array.isArray(coordinate) && coordinate.length >= 2 && typeof coordinate[0] === "number" && typeof coordinate[1] === "number" ? [[coordinate[0], coordinate[1]]] : []);
}
function routeSteps(value) {
    if (!Array.isArray(value))
        return [];
    return value.flatMap((segment) => isObject(segment) && Array.isArray(segment.steps) ? segment.steps : []).flatMap((step) => {
        if (!isObject(step) || typeof step.instruction !== "string")
            return [];
